...
void rbuff_init_dma_ro(rbuff_t *rbuff, void *buff, size_t capacity, size_t element_size, dma_w_cnt_get_cb dma_w_cnt_get)
{
    rbuff_init(rbuff, buff, capacity, element_size);
    rbuff->dma_ro        = true;
    rbuff->dma_w_cnt_get = dma_w_cnt_get;
}
...
size_t rbuff_peek(rbuff_t *rbuff, void *data, size_t size)
{
    ...
    while (cnt < size && !rbuff_is_empty(rbuff)) {
        memcpy(rbuff->r_ptr, (char *)data + (cnt * rbuff->element_size), rbuff->element_size);
        rbuff->r_ptr = rbuff_r_ptr_next(rbuff);
        cnt++;
    }
