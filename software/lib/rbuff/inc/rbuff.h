...
typedef void *(*dma_w_cnt_get_cb)(void);
...
typedef struct rbuff_tag {
    ...
    dma_w_cnt_get_cb dma_w_cnt_get; /**< Callback to get the DMA write pointer. */
} rbuff_t;
 ...
 * @param capacity The capacity of the ringbuffer in number of elements.
 * @param element_size The size of each element in the ringbuffer.
 * @param dma_w_cnt_get Callback to get the DMA write count.
 */
void rbuff_init_dma_ro(rbuff_t *rbuff, void *buff, size_t capacity, size_t element_size,
                       dma_w_cnt_get_cb dma_w_cnt_get);
